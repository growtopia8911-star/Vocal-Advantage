...
| 7 | Expected counts are wrong (steps say 21/40/57; actual 19/38/51). Scope each run to `tests/test_paste_win.py` and trust your own count.  **CONFIRMED and applied in `8b7dbb4`** - the counts really are 19/38/51. |
| 7 | In `test_blocked_sendinput_returns_false_but_still_releases_ctrl`, the assertion is `A or B` where `A` is always False. Drop the `or` and keep only the key-sequence assertion.  **APPLIED in `8b7dbb4`.** |
| 9 | `_start` sets `state = RECORDING` before `recorder.start()`. Task 5's `Recorder.start()` can raise `RecorderError` (mic unplugged) — set the state only after it returns, or the app sticks in RECORDING. **APPLIED in `b7cdfb6`** — `_start` now calls `recorder.start()` first and flashes `ERROR_MESSAGE` on failure instead of raising, because it runs on the thread draining the hotkey queue. Four tests cover it, all confirmed to fail against the original ordering. |
| 9 | `is_modifier_only` should mean "contains at least one bare modifier" (the spec says "is **or contains**"), not "every key is a modifier". Affects the cancel-on-other-key rule for mixed combos like `ctrl+space`. **NO CHANGE NEEDED** — Task 2 already shipped it as `any(k in MODIFIERS for k in self.keys)` with its own tests; verified 2026-08-19 and pinned by the controller's fixture-guard test. |
| 10 | `run_set_hotkey` must catch `TimeoutError` from `capture_hotkey` (Task 6 raises it when nothing is pressed) and print "Hotkey unchanged." — otherwise `--set-hotkey` ends in a stack trace.  **APPLIED in `d185ee7`**, without a retry: a timeout means nobody is at the keyboard. |
| 10 | `run_app` never calls `set_dpi_awareness()`. Task 8 requires it **before** `tk.Tk()` is constructed, or the pill renders blurry on a scaled display.  **APPLIED in `d185ee7`**, directly above `tk.Tk()`. |
| 10 | One test does a real `time.sleep(0.4)` and asserts wall-clock tick counts. Drive it with the injectable clock and a queue stub instead.  **APPLIED in `d185ee7`** - a queue stub burns its timeout on the injected clock; 8 expiries, 8 ticks, no sleeping. |

**All ten tasks are written and every correction in this table has been applied** (2026-08-19, all of it on the Mac — see the project note for why the Windows-only tasks turned out to be verifiable there). What remains is not code: it is the seven manual steps marked **NOT DONE** in Tasks 6, 7, 8 and 10, which need Windows, a microphone and a person.
...
- [x] **Step 5: Add the manual smoke test**
...
- [x] **Step 6: Run the manual smoke test against a real microphone** — run on macOS 2026-08-19: PASS, 30720 samples, float32, mono, peak 0.0315. **Still outstanding on the PC:** the Windows mic-in-use tray icon must be watched going ON then OFF, which is the only way to check that acceptance item.
...
- [x] **Step 1: Write the failing test — name translation**
...
- [x] **Step 3: Write the name-translation layer**
...
- [x] **Step 5: Write the failing test — the edge detector**
...
- [x] **Step 7: Write the edge detector**
...
- [x] **Step 9: Write the failing test — the listener**
...
- [x] **Step 13: Write the failing test — `--set-hotkey` capture**
...
- [ ] **Step 17: Manual smoke check (30 seconds, needs the real keyboard)**  **← NOT DONE: needs Windows. Outstanding for the PC.**
...
- [x] **Step 1: Write the failing test for the ctypes layout and constants**
...
- [x] **Step 3: Write the constants and ctypes structures**
...
- [x] **Step 5: Write the failing tests for the Windows layer**
...
- [x] **Step 7: Write the Windows layer**
...
- [x] **Step 9: Write the failing tests for the paste sequence**
...
- [x] **Step 11: Write the paste sequence**
...
- [x] **Step 12: Run the full suite to verify it passes**
...
- [ ] **Step 13: Manual smoke test (paste into Notepad)**  **← NOT DONE: needs Windows. Outstanding for the PC.**
...
- [x] **Step 3: Write the Windows half — constants, ctypes bindings, `Frame`, `_TkPill`**
...
- [x] **Step 4: Check the module loads and the bindings resolve**
...
- [x] **Step 5: Write the portable half — the queue and the state machine**
...
- [x] **Step 7: Write the manual harness**
...
- [ ] **Step 8: Manual checklist 1 of 3 — the focus checks (do these first)**  **← NOT DONE: needs Windows. Outstanding for the PC.**
...
- [ ] **Step 9: Manual checklist 2 of 3 — appearance and sequence**  **← NOT DONE: needs Windows. Outstanding for the PC.**
...
- [ ] **Step 10: Manual checklist 3 of 3 — the 150% DPI check**  **← NOT DONE: needs Windows. Outstanding for the PC.**
...
- [x] **Step 5: Run the whole suite to check nothing else moved**
...
- [x] **Step 6: Sanity-check the module in isolation**
...
- [x] **Step 3: Write the tested half of `vocal_advantage/main.py`**
...
- [x] **Step 4: Run the tests that do not need the entry point yet**
...
- [x] **Step 5: Append the app launcher to `vocal_advantage/main.py`, and create `__main__.py`**
...
- [x] **Step 6: Run the whole suite green**
...
- [ ] **Step 9: The acceptance run — walk this by hand**  **← NOT DONE: needs Windows. Outstanding for the PC.**
